    ...
    <div class="main">
        <table border="1">
            <thead>
                ...
                <th style="padding: 10px;">Husband</th>
                <th style="padding: 10px;" colspan="2">Operation</th>
            </thead>
<?php
$user= $_SESSION['username'];
$query= mysqli_query($conn, "SELECT * FROM fam_wife WHERE username= '$user'");
if ($query) {
    while ($row= mysqli_fetch_array($query)) {
?>
            <tr>
                <td style="padding: 10px;"><?php echo $row['wife_id'];?></td>
                <td style="padding: 10px;"><?php echo strtoupper($row['wife_firstname']);?></td>
                ...
                <td style="padding: 10px;"><?php echo strtoupper($row['husband']);?></td>
                <td style="padding: 10px;"><button><a href="operations.php?delete_wife=<?php echo $row['wife_id'];?>">Delete</a></button></td>
                <td style="padding: 10px;"><button><a href="operations.php?update_wife=<?php echo $row['wife_id'];?>">Update</a></button></td>
            </tr>
<?php
    }
}
